...
heterog_median <- read_excel("Output/heterog_median_migration.xlsx")
heterog_median_lapop <- read_excel("Output/heterog_median_migration_lapop.xlsx")
...
# ==========================================
# Constructor del panel de efectos heterogéneos (terciles)
# ==========================================
...
# ==========================================
# Constructor del panel de p-values  
# ==========================================
...
# ==========================================
# Constructor del panel de efectos heterogéneos (media)
# ==========================================
make_panel_tabular_median_from_stata <- function(df_panel, group_names, digits = 3) {
  stopifnot(length(group_names) == 2, length(unique(df_panel$label)) == 2)
  
  df_panel <- df_panel %>%
    mutate(label_order = match(label, group_names),
           group_order = match(group, c("Below", "Above"))) %>%
    arrange(label_order, group_order)
  ...
  header_top <- sprintf("& \\multicolumn{2}{c}{%s} & \\multicolumn{2}{c}{%s} \\\\",
                        group_names[1], group_names[2])
  cmid       <- "\\cmidrule(l){2-3} \\cmidrule(l){4-5}"
  header_sub <- "& Below & Above & Below & Above \\\\"
  ...
  c(
    ...
    "\\hline",
    header_top, cmid, header_sub, "\\hline",
    row_36, row_36_se, row_56, row_56_se,
    "\\addlinespace",
    row_n, row_p_eq,
    "\\hline",
    "\\end{tabular*}"
...
nota_completa <- paste0(
  "\\caption*{\\footnotesize Notes: The dependent variable is migration ",
    ...
    "municipal sample. The tercile variables are defined as follows. Population ",
    ...
    "pre-reform period. All specifications include municipality and year ",
...
# ---------------------------------------------------------------------------- #
# 6. Tablas efectos heterogéneos (variables de LAPOP)
# ---------------------------------------------------------------------------- #
{
# ==========================================
# Etiquetas y agrupamiento en paneles
...
tercile_labels <- tribble(
  ~tercile_var,          ~label,                             ~panel,
  "t_en_pareja",         "Share partnered",                    "A",
  "t_interes_pol_mucho", "Share very interested in politics",  "A",
)

heterog_labeled <- heterog_lapop %>% left_join(tercile_labels, by = "tercile_var")
...
panel_A <- make_panel_tabular_from_stata(
  df_panel    = heterog_labeled %>% filter(panel == "A"),
  group_names = c("Share partnered", "Share very interested in politics")
)
...
nota_completa <- paste0(
  "\\caption*{\\footnotesize Notes: The dependent variable is migration ",
  "intention. Each column reports subsample estimates for the tercile of the ",
  "specified municipal-level variable. Terciles are defined across municipalities " ,
  ...
  "bottom of each panel. The tercile variables are defined as follows: the share of ",
  "respondents who are very interested in politics and the share of respondents who ",
  "have a partner, both calculated at the municipality level by pooling individual-level ",
  "LAPOP responses across all pre-reform survey rounds. All specifications include municipality and year ",
  "fixed effects and individual controls for age and a male indicator. The reported coefficients are ",
  "average marginal effects from a logit specification, expressed as ",
  "the change in the probability of reporting migration intention per ",
  "one-unit increase in the corresponding regressor, where one unit ",
...
writeLines(final, "Output/migration_subsamples_lapop.tex")
}
# ---------------------------------------------------------------------------- #
# 7. Tabla de p valores de tests de igualdad de efectos entre terciles
# ---------------------------------------------------------------------------- #
{
# ==========================================
...
pairwise_bind <- rbind(pairwise, pairwise_lapop)
...
tercile_labels <- tribble(
  ...
  "t_interes_pol_mucho","Share interested in politics",   "E",
)

pairwise_labeled <- pairwise_bind %>% left_join(tercile_labels, by = "tercile_var")
...
panel_E_pv <- make_pvalue_panel_from_stata(
  ...
  group_names = c("Share partnered", "Share interested in politics")
)
...
final <- c(
  ...
  paste0(
    ...
    "Terciles are defined using the full ",
    "sample of 312 municipalities. The tercile variables are defined as follows. Population ",
    "density, the female share of the population, mean years of education, ",
    "median age, the labor-force participation rate among individuals aged 14 ",
    "and older, and the unemployment rate among individuals aged 14 and older ",
    "are measured using the 2010 census. Broad-left vote share is the average ",
    ...
    "changes relative to the previous election of the same type; the tercile ",
    "variable is the municipality-level average of this indicator over the 2011--2021 ",
    "pre-reform period.The share very interested in politics and ",
    "the share partnered are calculated at the municipality level ",
    "by pooling individual-level LAPOP responses across all pre-reform ",
    "survey rounds. * $p<0.10$, ** $p<0.05$, *** $p<0.01$.}"
  ),
  "\\end{table}"
)
...
# ---------------------------------------------------------------------------- #
# 8. Tablas efectos heterogéneos (median split)
# ---------------------------------------------------------------------------- #
{
# ==========================================
# Etiquetas y agrupamiento en paneles
# ==========================================
median_labels <- tribble(
  ~median_var,           ~label,                    ~panel,
  "p50_density_2010",    "Pop.\\ density",          "A",
  "p50_fem_2010",        "Female pop.",             "A",
  "p50_mean_schyr_2010", "Mean years educ.",        "B",
  "p50_med_dage_2010",   "Median age",              "B",
  "p50_pea_2010",        "Share in labor force",    "C",
  "p50_unemp_2010",      "Share unemployed",        "C",
  "p50_izam_pre_avg",    "Left vote share",         "D",
  "p50_alt_pre_avg",     "Ideological alternation", "D"
)

heterog_median_labeled <- heterog_median %>%
  mutate(group = ifelse(group == 1, "Below", "Above")) %>%
  left_join(median_labels, by = "median_var")

# ==========================================
# Construir los 4 paneles
# ==========================================
panel_A <- make_panel_tabular_median_from_stata(
  df_panel    = heterog_median_labeled %>% filter(panel == "A"),
  group_names = c("Pop.\\ density", "Female pop.")
)

panel_B <- make_panel_tabular_median_from_stata(
  df_panel    = heterog_median_labeled %>% filter(panel == "B"),
  group_names = c("Mean years educ.", "Median age")
)
first_hline_B <- grep("^\\\\hline$", panel_B)[1]
if (!is.na(first_hline_B)) panel_B <- panel_B[-first_hline_B]

panel_C <- make_panel_tabular_median_from_stata(
  df_panel    = heterog_median_labeled %>% filter(panel == "C"),
  group_names = c("Share in labor force", "Share unemployed")
)
first_hline_C <- grep("^\\\\hline$", panel_C)[1]
if (!is.na(first_hline_C)) panel_C <- panel_C[-first_hline_C]

panel_D <- make_panel_tabular_median_from_stata(
  df_panel    = heterog_median_labeled %>% filter(panel == "D"),
  group_names = c("Left vote share", "Ideological alternation")
)
first_hline_D <- grep("^\\\\hline$", panel_D)[1]
if (!is.na(first_hline_D)) panel_D <- panel_D[-first_hline_D]
...
# ==========================================
# Texto con los cutoffs de la mediana (leyendo group_ub del Excel)
# ==========================================
medians_text <- heterog_median_labeled %>%
  distinct(label, panel, group_ub) %>%
  arrange(panel, match(label, median_labels$label)) %>%
  mutate(entry = sprintf("%s (%.3f)", label, group_ub)) %>%
  pull(entry) %>%
  paste(collapse = "; ")

# ==========================================
# Texto con los tests pairwise Below vs Above
# ==========================================
pw_tests_text <- heterog_median_labeled %>%
  distinct(label, panel, p_pw_36, p_pw_56) %>%
  arrange(panel, match(label, median_labels$label)) %>%
  mutate(entry = sprintf("%s (%.3f / %.3f)", label, p_pw_36, p_pw_56)) %>%
  pull(entry) %>%
  paste(collapse = "; ")

nota_completa <- paste0(
  "\\caption*{\\footnotesize \\textit{Notes}: The dependent variable is ",
  "migration intention. Each column reports estimates for the subsample of ",
  "municipalities below or above the median of the specified municipal-level ",
  "variable. Medians are defined using the full sample of 312 municipalities, ",
  "and the corresponding median cutoffs used to split the sample are also ",
  "calculated from this full municipal sample. ",
  "The split variables are defined as follows. Population density, ",
  "the female share of the population, mean years of education, median age, ",
  "the labor-force participation rate among individuals aged 14 and older, ",
  "and the unemployment rate among individuals aged 14 and older are measured ",
  "using the 2010 census. Broad-left vote share is the average share of valid ",
  "votes received by left and center-left parties over the pre-reform ",
  "elections held between 2011 and 2021. Ideological alternation is an ",
  "indicator equal to one if the ideological bloc of the winning party ",
  "changes relative to the previous election of the same type; the split ",
  "variable is the municipality-level average of this indicator over the ",
  "2011--2021 pre-reform period. ",
  "The median value used to split the sample for each variable is: ",
  medians_text, ". ",
  "All specifications include municipality and year fixed effects and ",
  "individual controls for age and a male indicator. The reported ",
  "coefficients are average marginal effects from a logit specification, ",
  "expressed as the change in the probability of reporting migration ",
  "intention per one-unit increase in the corresponding regressor, where one ",
  "unit corresponds to one percentage point of the municipal Spanish-born ",
  "population share. Standard errors are clustered at the municipality level ",
  "and reported in parentheses (56 clusters). ",
  "The row labeled $p$-value: $\\beta^{1936-1955} = \\beta^{1956-1978}$ ",
  "reports the p-value from a two-sided Wald test of equality between the ",
  "two cohort-specific coefficients within each subsample. Tests of equality ",
  "of the coefficient on Spanish share $\\times$ Post between the ",
  "below-median and above-median subsamples ",
  "($H_0: \\beta_{\\text{Below}} = \\beta_{\\text{Above}}$), computed as ",
  "z-statistics on the difference between the two coefficients estimated on ",
  "independent subsamples, yield the following p-values, reported as ",
  "($\\beta^{1936-1955}$ / $\\beta^{1956-1978}$) for each variable: ",
  pw_tests_text, ". ",
  "* $p<0.10$, ** $p<0.05$, *** $p<0.01$. \\",
  "\\textit{Sources}:  LAPOP AmericasBarometer, 2023 round (dependent variables and individual controls) ",
  "National Electoral Commission (Left vote share, Ideological alternation); ",
  "IPUMS Argentine Census 1970 and 1980 (Spanish immigration shares); ",
  "IPUMS Argentine Census 2010 (municipal demographic and socioeconomic characteristics).}"
)
...
writeLines(final, "Output/migration_subsamples_median.tex")
}

# ---------------------------------------------------------------------------- #
# 9. Tablas efectos heterogéneos (median split, variables de LAPOP)
# ---------------------------------------------------------------------------- #
{
# ==========================================
# Etiquetas y agrupamiento en paneles
# ==========================================
median_labels_lapop <- tribble(
  ~median_var,               ~label,                              ~panel,
  "p50_en_pareja",           "Share partnered",                     "A",
  "p50_interes_pol_mucho",   "Share interested in politics",        "A",
  "p50_mujer",               "Share female",                        "B",
  "p50_edad",                "Median age",                          "B",
  "p50_desempleado",         "Share unemployed",                    "C",
  "p50_sec_completa",        "Share secondary educ.",               "C",
  "p50_izq_der",             "Mean left-right ideology",            "D",
  "p50_voto_blanco_nulo",    "Share blank/null vote",               "D"
)

heterog_median_lapop_labeled <- heterog_median_lapop %>%
  mutate(group = ifelse(group == 1, "Below", "Above")) %>%
  left_join(median_labels_lapop, by = "median_var")

# ==========================================
# Construir los 4 paneles
# ==========================================
panel_A <- make_panel_tabular_median_from_stata(
  df_panel    = heterog_median_lapop_labeled %>% filter(panel == "A"),
  group_names = c("Share partnered", "Share interested in politics")
)

panel_B <- make_panel_tabular_median_from_stata(
  df_panel    = heterog_median_lapop_labeled %>% filter(panel == "B"),
  group_names = c("Share female", "Median age")
)
first_hline_B <- grep("^\\\\hline$", panel_B)[1]
if (!is.na(first_hline_B)) panel_B <- panel_B[-first_hline_B]

panel_C <- make_panel_tabular_median_from_stata(
  df_panel    = heterog_median_lapop_labeled %>% filter(panel == "C"),
  group_names = c("Share unemployed", "Share secondary educ.")
)
first_hline_C <- grep("^\\\\hline$", panel_C)[1]
if (!is.na(first_hline_C)) panel_C <- panel_C[-first_hline_C]

panel_D <- make_panel_tabular_median_from_stata(
  df_panel    = heterog_median_lapop_labeled %>% filter(panel == "D"),
  group_names = c("Mean left-right ideology", "Share blank/null vote")
)
first_hline_D <- grep("^\\\\hline$", panel_D)[1]
if (!is.na(first_hline_D)) panel_D <- panel_D[-first_hline_D]
...
# Texto con los cutoffs de la mediana (leyendo group_ub del Excel)
# ==========================================
medians_text <- heterog_median_lapop_labeled %>%
  distinct(label, panel, group_ub) %>%
  arrange(panel, match(label, median_labels_lapop$label)) %>%
  mutate(entry = sprintf("%s (%.3f)", label, group_ub)) %>%
  pull(entry) %>%
  paste(collapse = "; ")
...
# Texto con los tests pairwise Below vs Above
# ==========================================
pw_tests_text <- heterog_median_lapop_labeled %>%
  distinct(label, panel, p_pw_36, p_pw_56) %>%
  arrange(panel, match(label, median_labels_lapop$label)) %>%
  mutate(entry = sprintf("%s (%.3f / %.3f)", label, p_pw_36, p_pw_56)) %>%
  pull(entry) %>%
...
nota_completa <- paste0(
  ...
  "migration intention. Each column reports estimates for the subsample of ",
  "municipalities below or above the median of the specified municipal-level ",
  "variable. Medians are defined across municipalities using the ",
  "pre-treatment average of each municipality-level aggregate over the 2012, ",
  "2014, 2017, and 2019 LAPOP survey waves, and the corresponding median ",
  "cutoffs used to split the sample are also calculated from this ",
  "pre-treatment sample. ",
  "The split variables are constructed at the municipality level by pooling ",
  "individual-level LAPOP responses across all pre-reform survey rounds and ",
  "are defined as follows: the share of respondents who have a partner; the ",
  "share of respondents who report being very interested in politics; the ",
  "share of female respondents; the mean age of respondents; the share of ",
  "unemployed respondents; the share of respondents with completed secondary ",
  "education or more; the mean self-placement on the left-right (1--10) ",
  "ideology scale; and the share of respondents who report having cast a ",
  "blank or null vote in the last presidential election. ",
  "The median value used to split the sample for each variable is: ",
  medians_text, ". ",
  "All specifications include municipality and year fixed effects and ",
  "individual controls for age and a male indicator. The reported ",
  ...
  pw_tests_text, ". ",
  "* $p<0.10$, ** $p<0.05$, *** $p<0.01$. ",
  "\\textit{Sources}:  LAPOP AmericasBarometer, 2023 round (dependent variables, individual controls and socioeconomic characteristics) ",
  "IPUMS Argentine Census 1970 and 1980 (Spanish immigration shares)}"
)
...
writeLines(final, "Output/migration_subsamples_median_lapop.tex")
}