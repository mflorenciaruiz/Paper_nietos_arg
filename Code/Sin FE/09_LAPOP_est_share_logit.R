# ---------------------------------------------------------------------------- #
#   LAPOP: Logit interacciones entre share español y características municipales
# ---------------------------------------------------------------------------- #
...
cat("Variables usadas para interacciones Logit con share español:\n")
print(interaction_vars)

# ---------------------------------------------------------------------------- #
# 5. Función para correr Logit X x share
# ---------------------------------------------------------------------------- #

run_share_interaction_logit <- function(x,
                                        with_controls = FALSE,
                                        data = lapop,
  ...
  cat("\nEstimando Logit interacción share español para:", x, "\n")
...
models_logit_share_interaction_no_controls <- map(
  interaction_vars,
  ~ run_share_interaction_logit(
    x = .x,
    with_controls = FALSE
  )
)

names(models_logit_share_interaction_no_controls) <- interaction_vars

models_logit_share_interaction_with_controls <- map(
  interaction_vars,
  ~ run_share_interaction_logit(
    x = .x,
    with_controls = TRUE
...
names(models_logit_share_interaction_with_controls) <- interaction_vars

# ---------------------------------------------------------------------------- #
# 7. Extraer solo coeficientes X x share
# ---------------------------------------------------------------------------- #

extract_share_interaction_logit_coefs <- function(model, x, controls_label) {
  ...
  tibble(
    ...
    or_1936_1955 = if (nrow(term_1936) > 0) {
      format_or(term_1936$estimate[1], term_1936$se[1], term_1936$p_value[1])
    } else {
      ""
    },
    or_1956_1978 = if (nrow(term_1956) > 0) {
      format_or(term_1956$estimate[1], term_1956$se[1], term_1956$p_value[1])
    } else {
      ""
    },
    nobs = nobs(model)
...
logit_share_interaction_table_no_controls <- map_dfr(
  names(models_logit_share_interaction_no_controls),
  function(x) {
    extract_share_interaction_logit_coefs(
      model = models_logit_share_interaction_no_controls[[x]],
      x = x,
      controls_label = "No"
    )
  }
...
logit_share_interaction_table_with_controls <- map_dfr(
  names(models_logit_share_interaction_with_controls),
  function(x) {
    extract_share_interaction_logit_coefs(
      model = models_logit_share_interaction_with_controls[[x]],
      x = x,
      controls_label = "Yes"
    )
  }
...
logit_share_interaction_table <- bind_rows(
  logit_share_interaction_table_no_controls,
  logit_share_interaction_table_with_controls
) %>%
  arrange(variable, controls)

print(logit_share_interaction_table)
...
logit_share_interaction_table_latex <- logit_share_interaction_table %>%
  mutate(
    variable = label_variable(variable),
    variable = latex_escape(variable),
    controls = latex_escape(controls),
    or_1936_1955 = latex_escape(or_1936_1955),
    or_1956_1978 = latex_escape(or_1956_1978)
  )
...
for (i in seq_len(nrow(logit_share_interaction_table_latex))) {
  latex_lines <- c(
    latex_lines,
    paste0(
      logit_share_interaction_table_latex$variable[i],
      " & ",
      logit_share_interaction_table_latex$controls[i],
      " & ",
      logit_share_interaction_table_latex$or_1936_1955[i],
      " & ",
      logit_share_interaction_table_latex$or_1956_1978[i],
      " & ",
      logit_share_interaction_table_latex$nobs[i],
      " \\\\"
    )
  )
}
...
writeLines(
  latex_lines,
  "Output/tex/logit_share_interactions_key_coefficients.tex"
)

cat("\nCódigo terminado correctamente.\n")
cat("Tabla Excel guardada en: Output/logit_share_interactions_key_coefficients.xlsx\n")
cat("Tabla LaTeX guardada en: Output/tex/logit_share_interactions_key_coefficients.tex\n")