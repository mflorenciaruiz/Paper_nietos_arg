# ---------------------------------------------------------------------------- #
#     LAPOP: Logit post x características municipales pre-2023
# ---------------------------------------------------------------------------- #
#
# Objetivo:
...
#   post:X
#
# La tabla reporta odds ratios: exp(coeficiente).
#
# ---------------------------------------------------------------------------- #
...
cat("Variables usadas para Logit post x X:\n")
print(post_x_vars)

# ---------------------------------------------------------------------------- #
# 5. Función para correr modelos Logit post x X
# ---------------------------------------------------------------------------- #
...
models_logit_post_x_no_controls <- map(
  post_x_vars,
  ~ run_post_x_logit(
    x = .x,
    with_controls = FALSE
...
names(models_logit_post_x_no_controls) <- post_x_vars

models_logit_post_x_with_controls <- map(
  post_x_vars,
  ~ run_post_x_logit(
    x = .x,
    with_controls = TRUE
...
names(models_logit_post_x_with_controls) <- post_x_vars

# ---------------------------------------------------------------------------- #
# 7. Extraer solo coeficientes post:X
# ---------------------------------------------------------------------------- #

extract_post_x_logit_coefs <- function(model, x, controls_label) {
  ...
  tibble(
    ...
    or_post_x = if (nrow(term_post_x) > 0) {
      format_or(term_post_x$estimate[1], term_post_x$se[1], term_post_x$p_value[1])
    } else {
      ""
    },
    nobs = nobs(model)
...
logit_post_x_table_no_controls <- map_dfr(
  names(models_logit_post_x_no_controls),
  function(x) {
    extract_post_x_logit_coefs(
      model = models_logit_post_x_no_controls[[x]],
      x = x,
      controls_label = "No"
    )
  }
...
logit_post_x_table_with_controls <- map_dfr(
  names(models_logit_post_x_with_controls),
  function(x) {
    extract_post_x_logit_coefs(
      model = models_logit_post_x_with_controls[[x]],
      x = x,
      controls_label = "Yes"
    )
  }
...
logit_post_x_table <- bind_rows(
  logit_post_x_table_no_controls,
  logit_post_x_table_with_controls
) %>%
  arrange(variable, controls)

print(logit_post_x_table)
...
for (i in seq_len(nrow(logit_post_x_table_latex))) {
  latex_lines <- c(
    latex_lines,
    paste0(
      logit_post_x_table_latex$variable[i],
      " & ",
      logit_post_x_table_latex$controls[i],
      " & ",
      logit_post_x_table_latex$or_post_x[i],
      " & ",
      logit_post_x_table_latex$nobs[i],
      " \\\\"
    )
  )
}
...
writeLines(
  latex_lines,
  "Output/tex/logit_post_x_key_coefficients.tex"
)

cat("\nCódigo terminado correctamente.\n")
cat("Tabla Excel guardada en: Output/logit_post_x_key_coefficients.xlsx\n")
cat("Tabla LaTeX guardada en: Output/tex/logit_post_x_key_coefficients.tex\n")