# ---------------------------------------------------------------------------- #
#     LAPOP: Logit triple diferencias por características municipales pre-2023
# ---------------------------------------------------------------------------- #
...
triple_vars <- unique(c(triple_vars_core, triple_vars_politics))
...
cat("Variables usadas para triples Logit:\n")
print(triple_vars)

# ---------------------------------------------------------------------------- #
# 5. Función para correr Logit triple diferencias
# ---------------------------------------------------------------------------- #
...
# ---------------------------------------------------------------------------- #
# 6. Estimar Logit triples sin controles y con controles
# ---------------------------------------------------------------------------- #

models_logit_triple_no_controls <- map(
  triple_vars,
  ~ run_triple_logit(
    x = .x,
    with_controls = FALSE
...
names(models_logit_triple_no_controls) <- triple_vars

models_logit_triple_with_controls <- map(
  triple_vars,
  ~ run_triple_logit(
    x = .x,
    with_controls = TRUE
...
extract_triple_logit_coefs <- function(model, x, controls_label) {
...
logit_triple_table_no_controls <- map_dfr(
  names(models_logit_triple_no_controls),
  function(x) {
    extract_triple_logit_coefs(
      model = models_logit_triple_no_controls[[x]],
      x = x,
      controls_label = "No"
    )
  }
...
logit_triple_table_with_controls <- map_dfr(
  names(models_logit_triple_with_controls),
  function(x) {
    extract_triple_logit_coefs(
      model = models_logit_triple_with_controls[[x]],
      x = x,
      controls_label = "Yes"
    )
  }
...
logit_triple_table <- bind_rows(
  logit_triple_table_no_controls,
  logit_triple_table_with_controls
) %>%
  arrange(variable, controls)

print(logit_triple_table)
...
logit_triple_table_latex <- logit_triple_table %>%
  mutate(
...
for (i in seq_len(nrow(logit_triple_table_latex))) {
  latex_lines <- c(
    latex_lines,
    paste0(
      logit_triple_table_latex$variable[i],
      " & ",
      logit_triple_table_latex$controls[i],
      " & ",
      logit_triple_table_latex$or_1936_1955[i],
      " & ",
      logit_triple_table_latex$or_1956_1978[i],
      " & ",
      logit_triple_table_latex$nobs[i],
      " \\\\"
    )
  )
}
...
writeLines(
  latex_lines,
  "Output/tex/logit_triple_differences_key_coefficients.tex"
)

cat("\nCódigo terminado correctamente.\n")
cat("Tabla Excel guardada en: Output/logit_triple_differences_key_coefficients.xlsx\n")
cat("Tabla LaTeX guardada en: Output/tex/logit_triple_differences_key_coefficients.tex\n")